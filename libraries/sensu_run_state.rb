  ...
  module ChefRunState
    ...
    def get_sensu_state(node, *keys)
      ...
      keys.inject(node.run_state['sensu']) do |memo, key|
        memo[key] if memo.is_a?(Hash) && Mash.new(memo).key?(key.to_s)
      end
    end
    ...
    def set_sensu_state(*args)
      ...
      real_value = value.is_a?(Chef::DataBagItem) ? value.to_hash.reject { |k, _v| bag_item_headers.include?(k) } : value

      set_key = args.pop
      leaf = args.inject(node.run_state['sensu']) do |memo, key|
        memo[key] = Mash.new unless memo[key].is_a?(Hash)
        memo[key]
      end
